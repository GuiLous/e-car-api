  ...
  module SessionServiceMutations
    class StartSession < BaseMutation
      ...
      def resolve(hired_service_id:)
        ...
        session_service = SessionService.where("? = ANY (hired_service_id)", hired_service_id).first
        session_service = assistant_start_session(hired_service_id, session_service) if current_user.assistant?
